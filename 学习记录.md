...
参考[设计层上的VO、DTO、DO、PO 概念、区别和作用](https://blog.csdn.net/weixin_43452424/article/details/107788166)

在尝试了docker之后，终于理解何为服务化的项目搭建，从而也理解了如何“只让API层调用业务层提供的服务接口”。单体项目中层与层之间是完全引用，一引全引，因此才有乱调用的可能性。服务化能完全解决该问题。

同样，原本为了便于开发，整个后端从底层的实体层，到顶层的API层都能拿到从数据库直接映射出来的模型（PO与DO统一，下称DO，毕竟领域驱动设计的核心是领域），但这样无法避免API层的程序员拿着模型乱搞（比如直接把模型作为DTO发出去，从而暴露数据库结构或者发出去什么不该发的东西）。

那将DO的持有限制到BLL层如何？这样的话，API层拿不到DO，DTO只能从BLL层获取，导致BLL层要处理DO到DTO的映射。

在了解了容器的通信机制后，决定采用BLL层处理DO到DTO的映射的方案，AutoMapper有用武之地了。
...
# 2024年9月10日
<!-- todo #14 -->
在修改代码的过程中发现数据库映射问题仍然存在，在参考建议后决定尝试启用数据库追踪。

<!-- todo #16 -->
在处理[#11](https://github.com/NEUQNPEE/NEUQBBS/issues/11)时认识到用户接口设计冗余，需要修改。

有必要学习一下 IEnumerable 以决定是否全面取代List。

byd……为什么UserDal没做接口……我真服了，修个bug越修越多。

## 思考

在修改获取用户信息相关代码时意识到一个问题：

在原本设计中，User被作为一个独立领域看待，User中有id、用户名、密码、签名、头像、注册时间、积分等等属性，单独在数据库里有一个Users表。

看似没有问题，但在做根据查询字段获取用户信息时，意识到User实际上设计成了复合领域：密码这一属性是属于登录领域的，而其他属性是属于用户信息领域的！

根据目前掌握的设计，在完成目前的各项修改后我们需要开始朝DDD方向进军。目前需要意识到：User中仍存在一个不应该出现到查询所得模型的Password属性，我们将在以后解决该问题。

# 2024年9月11日

在启动项目时出现警告：warning NU1900: 获取包漏洞数据时出错: 无法加载源 https://api.nuget.org/v3/index.json 。使用命令
...
将Blazor前端迁移到.net 8.0，尝试发布Blazor WebAssembly项目，发现需要动态部署服务器。暂停。